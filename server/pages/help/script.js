document.getElementById("markdownEmbed").addEventListener("load", () => {
  ...
  markdownStylesheet.innerHTML = ".crossnote { filter: hue-rotate(45deg); } ::selection { color: white; background-color: #00738b; } img { filter: hue-rotate(-40deg); } @media (prefers-color-scheme: dark) { img { filter: invert(95%) hue-rotate(180deg); } }";
...
window.addEventListener("message", ({ data: darkMode }) => {
  document.styleSheets[0].media.appendMedium("(prefers-color-scheme: " + ((darkMode) ? "dark" : "white") + ")");
});